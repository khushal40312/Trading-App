...
const TokenInfoPanel = React.memo(({ tradeRef, lastTrade, animate, token, imageSrc, priceRef, tradecoin, qtyRef, recentTrades }) => {
        ...
        <div className="w-[44vw] h-[70vh] bg-black text-white p-1 ">
            ...
            <div className='border border-white border-3 rounded  py-1 mt-3 h-70'>
                ...
                <div ref={priceRef} className='flex justify-center items-center py-3 gap-1  rounded bg-gray-600 border border-2 border-black mx-1'>
                    <h5
                        ...
                        className="text-xs text-white w-[50px] text-center  font-bold"
                    >

                        {tradeRef.current?.PRICE?.toString().startsWith('0.0')
                            ? `${tradeRef.current?.PRICE?.toFixed(5) || "---"}`
                            : `${tradeRef.current?.PRICE?.toFixed(2) || "---"}`}
                    ...
                    >
                        {tradeRef?.current?.LAST_TRADE_QUANTITY?.toString().startsWith('0.0')
                            ? tradeRef.current?.LAST_TRADE_QUANTITY?.toFixed(5)
                            : tradeRef.current?.LAST_TRADE_QUANTITY?.toFixed(2) || "---"}
                    </h5>
                ...
                <div className="">
                    {recentTrades?.slice().reverse().map((trade, idx) => (
                        <div key={idx} className={`flex justify-center items-center py-3 gap-1  rounded bg-black/40 border border-2 border-black m-1 ${trade.SIDE === 'buy' ? 'bg-green-500' : 'bg-red-500'}   `}>
                            {/* <span className={`font-semibold text-center text-xs w-2 h-2 m-1 rounded ${trade.SIDE === 'buy' ? 'bg-green-500' : 'bg-red-500'}`}> */}

                            {/* </span> */}
                            <span className={`text-xs text-black w-[50px] text-center  font-bold  `}>
                                {trade?.PRICE.toString().startsWith('0.0')
                                    ? `${trade?.PRICE.toFixed(5)}`
                                    : `${trade?.PRICE.toFixed(2)}`}

                            </span>
                            <span className=" font-bold text-black text-right  w-[50px] text-xs  w-1">
                                {trade.LAST_TRADE_QUANTITY.toString().startsWith('0.0')
                                    ? `${trade.LAST_TRADE_QUANTITY.toFixed(5)}`
                                    : `${trade.LAST_TRADE_QUANTITY.toFixed(2)}`}
            ...
            <div className="flex flex-col items-center ">
                ...
                >
                    {tradeRef.current?.PRICE?.toString().startsWith('0.0')
                            ? `${tradeRef.current?.PRICE?.toFixed(5) || "---"}`
                            : `${tradeRef.current?.PRICE?.toFixed(2) || "---"}`} <span className="text-sm">USDT</span>
                </h5>
            </div>
        </div>
    )
